...
flags.DEFINE_boolean('collisionwarning', False, 'outputs warning is object is in path')
...
def main(_argv):
    ...
    while True:
        ...
        if FLAGS.collisionwarning:
            #####warning instance detected
            collision_warning = collision_warning(pred_bbox, stay_safe = True)
            
            image = utils.draw_bbox(frame, pred_bbox, FLAGS.info, collision_warning, allowed_classes=allowed_classes, read_plate=FLAGS.plate)    
        else:
            image = utils.draw_bbox(frame, pred_bbox, FLAGS.info, allowed_classes=allowed_classes, read_plate=FLAGS.plate)
